...
export const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      // Upload WebP binary file to Supabase Storage CDN for 10x faster uploads
      const { uploadMediaToStorageCDN } = await import("@/lib/utils/fast-media-uploader");
      const storageRes = await uploadMediaToStorageCDN(optResult.file, file.name || "image");

      if (storageRes.success && storageRes.url) {
        optResult.dataUrl = storageRes.url;
      }
      ...
      toast.success(`Image uploaded to CDN! ${savings > 0 ? `Saved ${savings}% file size.` : ""}`);
    } catch (err) {
      console.error(err);
