  ...
  const verifyCode = async (rawCode: string) => {
    ...
    try {
      ...
      if (cleanCode.startsWith("VOLUNTEER:")) {
        ...
        const volId = parts[1];
        const { data: volData } = await supabase.from("volunteers").select("*").eq("id", volId).maybeSingle();
      ...
      const { data: reg, error } = await supabase
        ...
        .or(`pass_code.eq.${targetPassCode},id.eq.${targetPassCode}`)
        .maybeSingle();
          ...
          {!result && (
            <div className="space-y-4">
              <div className="relative overflow-hidden rounded-3xl border-2 border-primary/40 bg-black min-h-[260px] grid place-items-center">
                ...
                {!cameraActive && (
                  <div className="p-6 text-center text-white space-y-3">
                    ...
                    <p className="text-sm font-semibold">Point camera at attendee's QR Pass to verify instantly</p>
                    <Button
              ...
              <form onSubmit={handleManualSubmit} className="flex gap-2">
                ...
                />
                <Button disabled={loading} type="submit" className="rounded-2xl gradient-saffron text-primary-foreground shrink-0 text-xs font-bold">
            ...
            <div className="space-y-5 animate-in fade-in zoom-in-95 duration-200">
              ...
              {result.isValid ? (
                <div className="rounded-3xl bg-emerald-500/15 border-2 border-emerald-500/40 p-4 text-emerald-600 dark:text-emerald-400 text-center space-y-1">
                  ...
                  <p className="text-xs font-medium text-emerald-600 dark:text-emerald-400">{result.message}</p>
                </div>
              ) : (
              ...
              {result.registration && (
                <div className="card-premium p-5 space-y-4">
                  <div className="flex items-center justify-between border-b border-border pb-3">
                    <div>
                      <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Attendee Name</span>
                      <h4 className="font-display text-lg font-bold text-foreground">{result.registration.full_name}</h4>
                    </div>
                  ...
                  <div className="grid gap-3 text-xs sm:grid-cols-2">
                    <div className="rounded-2xl bg-secondary p-3 border border-border">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase block">Phone / Contact</span>
                      <span className="font-semibold">{result.registration.phone}</span>
                    </div>

                    <div className="rounded-2xl bg-secondary p-3 border border-border">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase block">Registration Status</span>
                  ...
                  <div className="rounded-2xl bg-primary/10 p-4 border border-primary/20 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Attendance Check-In</span>
                      {result.registration.attended ? (
                        <Badge className="rounded-full bg-emerald-600 text-white font-bold">
                          ✓ ALREADY CHECKED-IN
                        </Badge>
                      ) : (
                        <Badge variant="outline" className="rounded-full text-amber-600 border-amber-500 font-bold">
                          NOT CHECKED-IN YET
                        </Badge>
                      )}
                    </div>
                    ...
                    {!result.registration.attended && (
                      ...
                      >
                        {markingAttendance ? "Marking Check-In..." : "Mark Attendance / Confirm Entry"}
                      </Button>
                    )}
                  </div>
                </div>
              ...
              {result.volunteer && (
                <div className="card-premium p-5 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className="rounded-full bg-emerald-600 text-white font-bold">VERIFIED VOLUNTEER</Badge>
                  </div>
                  <div className="grid gap-2 text-xs">
                    ...
                    <div className="flex justify-between rounded-xl bg-secondary p-2.5">
                      ...
                      <span className="font-semibold text-primary">{result.volunteer.duty || "General Seva"}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="flex gap-2">
                <Button onClick={handleResetScan} variant="outline" className="w-full rounded-full font-bold">
