...
export function QrPass({ registration }: { registration: Registration }) {
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(payload, { width: 480, margin: 1, color: { dark: "#1a0f06", light: "#ffffff" } })
      .then(setQr)
      .catch(() => setQr(""));
  }, [registration]);
    ...
    <article className="card-premium overflow-hidden">
      ...
      <div className="grid gap-5 p-5 sm:grid-cols-[auto_minmax(0,1fr)]">
        {qr ? (
          ...
          <div className="h-35 w-35 rounded-2xl skeleton-shimmer" style={{ width: 140, height: 140 }} />
        )}
        ...
        <dl className="grid gap-2 text-sm">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            <Badge className="rounded-full" variant={registration.status === "confirmed" ? "default" : "secondary"}>
              {registration.status}
            </Badge>
...
function Row({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="grid grid-cols-[80px_minmax(0,1fr)] gap-2">
      ...
      <dd className={`truncate font-medium ${mono ? "font-mono tracking-wider text-primary" : ""}`}>{value}</dd>
    </div>
  );
}
