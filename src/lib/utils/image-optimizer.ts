  ...
  return new Promise((resolve) => {
    ...
    reader.onload = () => {
      ...
      img.onload = async () => {
        ...
        let blurDataUrl = "";
        try {
          blurDataUrl = await generateLQIPFromCanvas(canvas);
        } catch {
          // Ignore blur fallback
        }
      ...
      img.onerror = () => {
        // Fail-safe: Return original file if image element fails parsing
        resolve({
          file,
          dataUrl: srcDataUrl,
          width: 800,
...
export function buildResponsiveSrcSet(
  src: string,
  widths: number[] = [360, 640, 960, 1280, 1920],
): string {
  if (!src) return "";
